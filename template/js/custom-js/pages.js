    ...
    $(el || '#content .product-card[data-product-id='+ _id +']').each(function(){
        ...
        if(grid){
            ...
            $.each(grid,function(k,item){
                let block = $('<div key="'+k+'" class="apx_fastBuy-options"></div>');
...
$('body').on('click','.apx_fastBuy button', function(){
    let product = JSON.parse($(this).closest('[data-product-full]').attr('data-product-full'));
    product.product_id = product._id;
    $(this).closest('div').find('button').removeClass('active');
    $(this).addClass('active');
    let fb = $(this).closest('.apx_fastBuy');    
    ...
    if(fb.find('[key]').length == 2){
        if($(this).closest('[key]').is(':last-child')){
            ...
            let grid = JSON.parse($(this).closest('.apx_fastBuy').attr('grid'));
            let key = $(this).closest('[key]').attr('key');
            let current_opt = $(this).attr('option');
