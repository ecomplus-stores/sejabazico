...
export default {
  ...
  methods: {
    ...
    fetchProduct (isRetry = false) {
      ...
      store({
        ...
        .then(({ data }) => {
          ...
          window.$('.isMobile #page-products #product-actions .product__prices').appendTo('#bz_product_mobile .__prices')
        })
        .catch(err => {
          console.error(err)
          const { response } = err
    ...
    changeQntUn (item, qntDiff, ev) {
      const { body } = this
      const input = ev.target.closest('.apx_quantity_selector').querySelector('input')
